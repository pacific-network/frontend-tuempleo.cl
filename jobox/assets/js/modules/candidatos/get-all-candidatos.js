document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchPostulantes(page = 1) {
    try {
      ...
      candidatosContainer.innerHTML = `<div class="alert alert-danger">Aún no hay postulantes.</div>`;
    }
  }
  ...
  function renderPostulantes(postulantes) {
    if (!postulantes || postulantes.length === 0) {
      candidatosContainer.innerHTML = `<p class="text-center text-muted mt-3">Aún no hay postulantes.</p>`;
      return;
    }
