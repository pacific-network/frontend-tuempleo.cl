...
// ===============================
// ⚡ Toast reutilizable (SweetAlert2)
// ===============================
...
function getUserIdFromToken() {
  ...
  try {
    ...
    return d.sub || d.userId || d.id || null;
  } catch {
    return null;
  }
}
...
function getFreeRemaining() {
  ...
  if (val === null || isNaN(+val)) {
    localStorage.setItem(key, String(FREE_MONTHLY_LIMIT));
      ...
      <div class="pricing-item ${p.popular ? "active" : ""}">
        ...
        <div class="pricing-content">
          ...
          <div class="pricing-amount">
            ${
              p.precioLista
                ? `<span class="price-list">${fmtCLP(p.precioLista)}</span>`
                : ""
            }
          ...
          ${
            isFree
              ? `<div class="free-badge"><i class="fa fa-gift"></i> Gratis disponibles: <strong>${freeLeft}/${FREE_MONTHLY_LIMIT}</strong></div>`
              : ""
          }
        </div>
        <div class="pricing-feature"><ul>${p.caracteristicas
          .map((c) => `<li>${c}</li>`)
          .join("")}</ul></div>
        <button class="theme-btn" data-id="${p.id}" ${
  ...
  cartItemsBox.innerHTML = carrito
    ...
    <div class="row">
      ...
      <div class="qty">
        ...
        <button class="btn-remove" data-id="${i.id}" title="Eliminar">🗑️</button>
      </div>
