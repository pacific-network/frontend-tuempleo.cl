...
async function loadCualificados() {
  try {
    const data = await getDescartados(OFERTA_ID);
    console.log("📌 Cualificados:", data);
    renderDescartados(data || []);
    attachVerCvEvents(); 
  } catch (error) {
    console.error("❌ Error cargando descartados:", error);
  }
}
...
function renderDescartados(lista) {
  const contenedor = document.getElementById("lista-descartados");
  const empty = document.getElementById("empty-descartados");
  const count = document.getElementById("count-descartados");
  ...
  lista.forEach(item => {
    contenedor.innerHTML += CrearCardDescartados(item);
  });
}
...
// =============================
function CrearCardDescartados(item) {
