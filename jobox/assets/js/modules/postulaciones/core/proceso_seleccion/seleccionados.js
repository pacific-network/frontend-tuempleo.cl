import { getSeleccionados } from "../api-postulaciones.js"
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    initSeleccionados(id);
});

export async function initSeleccionados(ofertaId){
    OFERTA_ID = ofertaId;
    await loadSeleccionados();
}

async function loadSeleccionados(){
    try{
        const data = await getSeleccionados(OFERTA_ID);
        console.log("📌 Seleccionados:", data);
        renderSeleccionados(data || []);
        attachVerCvEvents();
    } catch(error){
        console.error("❌ Error cargando seleccionados:", error);
    }
}

export function renderSeleccionados(lista){
    const contenedor = document.getElementById("lista-seleccionados");
    const empty = document.getElementById("empty-seleccionados");
    const count = document.getElementById("count-seleccionados");
    ...
    lista.forEach(item => {
        ...
        div.innerHTML = crearCardSeleccionado(item);
...
export function attachVerCvEvents() {
  ...
  function crearCardSeleccionado(item) {
