...
async function renderOfertas(page = 1) {
  ...
  try {
    // 🔥 Se agrega page y take correctamente
    const res = await fetch(
      `${BASE_URL_API}/ofertas/empleador/${empleador_id}?page=${page}&take=${itemsPerPage}`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    // Datos paginados correctamente desde backend
    let ofertas = json.data || [];
    ...
    const filas = await Promise.all(
      ofertas.map(async (oferta) => {
        ...
        try {
          data =
            oferta.data && typeof oferta.data === "string"
              ? JSON.parse(oferta.data)
              : oferta.data || {};
        } catch {
          data = {};
        }
        ...
        if (data.renta_salarial) {
          const d = Number(data.renta_salarial.desde || 0);
          const h = Number(data.renta_salarial.hasta || 0);
          const f = (n) => n.toLocaleString("es-CL");
